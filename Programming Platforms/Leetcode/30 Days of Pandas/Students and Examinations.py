...
def students_and_examinations(students: pd.DataFrame, subjects: pd.DataFrame, examinations: pd.DataFrame) -> pd.DataFrame:
    def check(row, df):
        filt = (df['student_id'] == row['student_id']) & (df['subject_name'] == row['subject_name'])
        return len(df[filt])
    exam = students.merge(subjects, how='cross').sort_values(by=['student_id','subject_name'])
    exam['attended_exams'] = exam.apply(lambda x: check(x,examinations), axis=1)
    return exam
