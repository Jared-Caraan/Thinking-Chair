...
def dayOfProgrammer(year):
    ...
    if year < 1919 and year != 1918:
        if year % 4 == 0 and year % 100 == 0:
            return (date(year, 1, 1) + timedelta(days=254)).strftime('%d.%m.%Y')
    elif year == 1918:
            return (date(year, 1, 1) + timedelta(days=268)).strftime('%d.%m.%Y')
    
    return (date(year, 1, 1) + timedelta(days=255)).strftime('%d.%m.%Y')